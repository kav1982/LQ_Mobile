Shader "MMN/CH/Skin_Face" {
	Properties {
		...
		[Header(Skin Dye Color)] [Space(10)] _DyeColor1 ("피부 염색 슬롯 1", Vector) = (1,1,1,1)
		_DyeColor2 ("피부 염색 슬롯 2", Vector) = (1,1,1,1)
		_DyeColor3 ("피부 염색 슬롯 3", Vector) = (1,1,1,1)
		[Header(Mouth)] [Space(10)] [Enum(Base, 0, Emotion, 1)] _MouthShowType ("보여줄 입 맵", Float) = 0
		[NoScaleOffset] _MouthMap ("기본 입 맵", 2D) = "black" {}
		_MouthMapScalePosition ("입 크기(XY), 위치(ZW)", Vector) = (1,1,0,0)
		_MouthMapRotation ("입 회전", Float) = 0
		[Space(10)] [NoScaleOffset] _EmotionMouthMap ("이모션 입 맵", 2D) = "black" {}
		_EmotionMouthMapAtlasSize ("아틀라스 컬럼(X) 로(Y) 인덱스(Z) 회전(W)", Vector) = (1,1,1,0)
		_EmotionMouthMapScalePositionBase ("[기본값] 이모션 입 크기(XY), 위치(ZW)", Vector) = (1,1,0,0)
		[HideInInspector] _EmotionMouthMapScalePosition ("!!애니메이션 전용!! 이모션 입 크기(XY), 위치(ZW)", Vector) = (1,1,0,0)
		[Space(10)] _MouthPushStrength ("입 밀어넣기 정도", Range(0, 1)) = 0.6
		...
		_TattooPushStrength ("문신/흉터 밀어넣기", Range(0, 1)) = 0
		...
		[Header(Beard)] [Space(10)] [NoScaleOffset] _BeardMap ("수염 맵", 2D) = "black" {}
		_BeardMapScalePosition ("크기(XY), 위치(ZW)", Vector) = (1,1,0,0)
		_BeardMapRotation ("회전", Float) = 1
		[Toggle] _IsDyableBeard ("염색 마스크 포함?", Float) = 0
		_BeardDyeColor1 ("염색 1", Vector) = (1,1,1,1)
		_BeardDyeColor2 ("염색 2", Vector) = (1,1,1,1)
		_BeardDyeColor3 ("염색 3", Vector) = (1,1,1,1)
		[Header(Accessory)] [Space(10)] [NoScaleOffset] _AccessoryMap ("악세서리 맵(스티커)", 2D) = "black" {}
		[Toggle] _IsDyableAccessory ("염색 마스크 포함?", Float) = 0
		_AccessoryDyeColor1 ("염색 1", Vector) = (1,1,1,1)
		_AccessoryDyeColor2 ("염색 2", Vector) = (1,1,1,1)
		_AccessoryDyeColor3 ("염색 3", Vector) = (1,1,1,1)
