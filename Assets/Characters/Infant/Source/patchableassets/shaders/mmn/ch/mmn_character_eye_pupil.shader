Shader "MMN/CH/Eye_Pupil" {
	Properties {
		[Header(Base Map)] [Space(10)] _BaseMap ("베이스 맵", 2D) = "white" {}
		_BaseMapAtlasSize ("아틀라스 행(X) 열(Y) 인덱스(Z) 회전(W)", Vector) = (1,1,1,0)
		_BaseMapScalePosition ("크기(XY), 위치(ZW)", Vector) = (1,1,0,0)
		...
		[HDR] _EmissionColor ("이미션 컬러", Vector) = (0,0,0,1)
		_EmissionIntensity ("이미션 강도", Range(0, 10)) = 1
