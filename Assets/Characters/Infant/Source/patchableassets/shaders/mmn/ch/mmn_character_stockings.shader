Shader "MMN/CH/Stockings" {
	Properties {
		[Header(Texture)] [Space(10)] _BaseMap ("베이스 맵", 2D) = "black" {}
		_TintColor ("틴트 색상", Vector) = (1,1,1,1)
		[Header(Dye Mask Texture)] [Space(10)] [Toggle] _UseDyeMaskMap ("염색 마스크 맵 사용?", Float) = 0
		_DyeMaskMap ("염색 마스크 맵", 2D) = "black" {}
		[Header(Dye Color)] [Space(10)] _DyeColor1 ("염색 슬롯 1 (피부)", Vector) = (1,1,1,1)
		_DyeColor2 ("염색 슬롯 2 (스타킹_메인)", Vector) = (1,1,1,1)
		_DyeColor3 ("염색 슬롯 3 (스타킹_포인트)", Vector) = (1,1,1,1)
		[Header(Stockings)] [Space(10)] _Denier ("스타킹 두께감", Range(10, 180)) = 60
		_PointColorSpecular ("포인트 컬러 반짝임", Range(0, 1)) = 0
